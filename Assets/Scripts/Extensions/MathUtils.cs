...
namespace Utils.Extensions
{
    public static class MathUtils
    {
        ...
        public static float Progression(this float n, int level, float power)
        {
            if (level == 0) return 0f;
            return n + Mathf.Pow(level, power);
        }

        public static ulong GetPowULong(ulong requiredULong, int pow)
        {
            var originalULong = requiredULong;
            requiredULong = 1;
            for (var i = 0; i < pow; i++)
            {
